...
// Set up the query (empty b/c not sending 'q')
    var query_params = _.extend({}, options);
...
var addEntityValue = function (access_token, entity, value, expressions, options, callback) {
    if(!callback) {
        callback = options;
        options = undefined;
    }
    
// Set up the query, empty object
    var query_params = _.extend({}, options);

// Request options
    var request_options = {
        url: 'https://api.wit.ai/entities/'+ entity + '/values?v=20160526',
        headers: {
             'Authorization': 'Bearer ' + access_token,
...
module.exports.addEntityValue = addEntityValue;
